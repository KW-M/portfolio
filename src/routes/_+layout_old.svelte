<script lang="ts">
  import "./layout.css";
  import favicon from "$lib/assets/favicons/favicon.ico";
  ...
  import TagNav from "$components/categoryNav.svelte";
  ...
  let mainNavOpen = $derived.by(() => navOpen || page.route.id === "/");
  let pageNavOpen = $derived.by(() => navOpen && page.route.id != "/");
...
<nav class="fixed border-nav pointer-events-none z-30 duration-300" class:nav-open={mainNavOpen}>
  <svg viewBox="0 0 60 60" xmlns="http://www.w3.org/2000/svg" fill-rule="evenodd" clip-rule="evenodd" stroke-linejoin="round" stroke-miterlimit="2" class="absolute -bottom-[1px] -left-[1px] w-40 h-40 duration-300 ease-in-out transition-transform origin-bottom-left scale-50">
    <path d="M60 0v60H0c33.115 0 60-26.885 60-60z" transform="rotate(90)" transform-origin="center center" />
  </svg>
  <svg viewBox="0 0 60 60" xmlns="http://www.w3.org/2000/svg" fill-rule="evenodd" clip-rule="evenodd" stroke-linejoin="round" stroke-miterlimit="2" class="absolute -bottom-[1px] -right-[1px] w-40 h-40 duration-300 ease-in-out transition-transform origin-bottom-right scale-50">
...
{#if $previewZoomOpen}
  <div on:click={() => previewZoomOpen.set(false)} aria-hidden="true" id="img_zoom_backdrop bg-black" class="z-40" transition:fade={{ duration: TRANSITION_DURRATION }}></div>
