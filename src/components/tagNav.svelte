...
<div class="bottom-0 left-0 right-0 pr-20 pl-32 z-40 h-20 fixed text-black flex-row flex items-center justify-center overflow-x-auto overflow-y-hidden transition-all duration-300" class:translate-y-full={!open} class:opacity-0={!open} on:mouseenter={() => disableBrowserBackSwipe.set(true)} on:mouseleave={() => disableBrowserBackSwipe.set(false)} role="navigation" aria-label="Category navigation">
...
<div class="fixed left-0 bottom-0 box-border h-16 border-l-[50px] border-black border-solid w-40 bg-gradient-to-r from-black to-transparent z-40 pointer-events-none transition-all duration-300" class:translate-y-full={!open} class:opacity-0={!open}></div>
