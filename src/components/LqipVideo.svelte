...
<script lang="ts">
  ...
  interface Props {
    video: { type: "video"; title?: string; formats: { src: string; type: string }[]; src: string; width: number; height: number; lqip: string };
    loadHiRez?: boolean;
    onClick?: any;
    isCentered?: boolean;
    class?: string;
  ...
  let { video, loadHiRez = true, onClick = () => true, isCentered = false, class: className = "", rounded = true, showZoom = false }: Props = $props();
  // };
  ...
  const onclickcapture = (e: Event) => {
    ...
    if (onClick() && showZoom && videoElement && !videoElement.paused) {
      zoomed = !zoomed;
    }
...
<div class="h-full relative bg-transparent cursor-zoom-in rounded-xl overflow-hidden" style={`aspect-ratio: ${video.width}/${video.height}`}>
  <figure use:attachZoom={{ zoomed, width: video.width, height: video.height }} style={`background-image: url(${video.lqip}); aspect-ratio: ${video.width}/${video.height}; animation-delay: ${-instanceNum * 500}ms;`} class="blurable rounded-xl overflow-hidden block h-full w-full bg-cover relative animate-pulse" class:blur={!loaded} class:animate-pulse={!loaded} {onmouseenter} {onmouseleave}>
    <!-- <button
      class="w-full h-full"
    ...
    {#if loaded && showZoom && !zoomed}
      <button
        ...
        tabindex="-1"
        aria-hidden="true"
        class="absolute top-4 right-4 btn opacity-0 btn-icon-lg cursor-pointer zoom-icon-btn preset-filled-surface-100-900 hover:shadow-xl bg-surface-100-900/80 backdrop-blur-md text-surface-contrast-50 dark:bg-surface-950/60 dark:text-surface-contrast-950 z-10"
